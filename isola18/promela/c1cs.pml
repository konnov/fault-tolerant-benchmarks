/**
 * Consensus in One Communication Step (crash faults).
 *
...
/*
  BUG1 introduces a safety bug,
  BUG2 introduces a liveness bug
 */
    ...
    assume(F > 1);
        ...
        /* case 1 */
        assume(F == 0);
        #pragma option "piaDom.thresholds=0,1,T+1,N-2*T,N-T"
    #endif
#endif
        ...
        :: atomic {
            ...
            if
            	...
            	if
                    ...
                    :: (pc == S0 || pc == S1) && next_nrcvd0 + next_nrcvd1 >= N - T ->
                        if
                            /* decide immediately */
                            :: next_nrcvd1 == 0 -> next_pc = D0;

                            :: next_nrcvd0 == 0 -> next_pc = D1;

                            :: next_nrcvd0 >= 1 && next_nrcvd1 >= 1 ->
                                if
                                    /* set the value for
...
#ifdef BUG2
                    /* introduce a simple liveness bug: the process never decides */
                    :: pc == S0 || pc == S1 ->
                        next_pc = pc;
#endif
...
#ifdef BUG1
                    /* There is not much what we can break in the safety
                       with the crash faults. Introduce a bug in the code. */
...
ltl fairness { []<>(!in_transit0 && !in_transit1) }
    ...
    ltl one_step_almost0 { almost_all_at_V0 ->
        [](all_not_at_D1 && all_not_at_U0 && all_not_at_U1) }

    ltl one_step_almost1 { almost_all_at_V1 ->
        [](all_not_at_D0 && all_not_at_U0 && all_not_at_U1) }
