...
symbolic int NplusTdiv2; /* auxillary parameter */

int nsnte, nsntr;
/* express integer division without using it, as MathSAT complains */
assume((2 * NplusTdiv2 == N + T) || (2 * NplusTdiv2 == N + T - 1));
...
assume(N >= 3 * T); /* BUG2=1 */
    ...
    #endif
#else
    assume(F <= T + 1); /* FeqTp1=1 */
#endif

/* we have to consider two orders here */
#ifdef CASE2
/* case 2 */
assume((NplusTdiv2 + 1) > (2 * T + 1)) 
#else
/* case 1 */
assume((NplusTdiv2 + 1) == (2 * T + 1)) 
#endif
...
atomic disabled1 =
    (some(/*Proc:pc == V0 || Proc:pc == V1 || Proc:pc == EC || */Proc:pc == RD)
        and (nsntr >= 2 * T + 1));

atomic disabled2 =
    (some(Proc:pc == V0) and (nsnte >= NplusTdiv2 + 1));

atomic disabled3 =
    (some(Proc:pc == V0 || Proc:pc == EC) and (nsntr >= T + 1));

atomic fairV1 = all(Proc:pc != V1);
        ...
        :: atomic {
            ...
            if
            	...
            	if
                    ...
                    :: (pc == V0) && (next_nrcvde >= NplusTdiv2 + 1) ->
                    	next_pc = EC;

                    :: pc == V0 && (next_nrcvdr >= T + 1) ->
                    	next_pc = EC;

               	    :: (pc == EC) && (next_nrcvde >= NplusTdiv2 + 1) ->
                    	next_pc = RD;                   

                    :: (pc == EC) && (next_nrcvdr >= (T + 1)) ->
                        next_pc = RD;
...
#else
    ltl agreement { [](ex_acc -> <>(all_acc))} /* L2 */
    ltl corr { (prec_corr -> <>(ex_acc)) }     /* L1 */
    ltl unforg { (prec_unforg -> []!ex_acc) }  /* S1 */
    
    /* simplified specifications */
    ltl completeness { (prec_corr -> <>(all_acc)) }
    ltl agreement_all0 { prec_unforg -> [](ex_acc -> <>(all_acc))}
    ltl agreement_all1 { prec_corr -> [](ex_acc -> <>(all_acc))}
#endif
