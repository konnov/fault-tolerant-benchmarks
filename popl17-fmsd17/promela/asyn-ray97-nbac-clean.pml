    ...
    do
        :: atomic {
          ...
          if
            ...
            :: pc != CRASH && pc != CRASH_NO &&
                    pc != YES && pc != NO
                    && (pc != SEND ||
                   ((next_nrcvdNo == 0) && next_some_fail == 0))
                    && (pc != SEND ||
                   ((next_nrcvdNo != 0) || (next_nrcvdYes < N))) ->
               next_pc = pc;
            ...
            :: pc == SEND && next_nrcvdNo == 0 && next_nrcvdYes >= N ->
                next_pc = SEND;
#endif
...
ltl fairness { []<>(!in_transit_yes && !in_transit_no) }
    ...
    /* needs a complete consensus algorithm:
...
#else
    ltl termination1 { [] all_not_crashed -> <> all_decide}
    ltl termination2 { <>[] all_suspicious -> <> all_decide}
    /* needs a complete consensus algorithm:
    ltl agreement { [] (!(some_commit && some_abort))} */
    ltl validity { (all_yes || ([] !some_commit)) }
    ltl nontriv { ((all_yes && ([] all_not_suspicious)
            && ([] all_not_crashed)) -> <> all_commit) }
#endif
