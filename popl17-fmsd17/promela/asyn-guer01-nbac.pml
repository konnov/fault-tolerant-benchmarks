...
/* BUG1 is an artificially injected bug (see below) that violates safety */
/* BUG2 is an artificially injected bug (see below) that violates liveness */
  ...
  if
    ...
    do
        :: atomic {
          ...
          if
            :: pc == NO || pc == YES ->
              /* Ask the failure detector.
                Its output is restricted with an LTL formula */
              next_nrcvdNo = nrcvdNo;
              next_nrcvdYes = nrcvdYes;
              /* In this paper, the failure detector
                 knows initially, whether a process has crashed or not.
                 So, we do not change the outcome of the failure detector */
...
#ifdef BUG2
                /* a simple liveness bug: the process never commits */
            :: pc == SENT && next_nrcvdNo == 0 && next_nrcvdYes >= N ->
                next_pc = SENT;
#endif
...
/*
   If initially, at least process votes against commiting,
   then no process commits
 */
ltl abort_validity { (some_no -> ([] !some_commit)) }
/* If initially, all processes vote for yes and the failure detector
   reports no crashes, then no process aborts.
   Note that in our modeling, processes can still crash, even if
   the failure detector foresees no crashes. This does not change
   the safety property, as nobody aborts in this case.
 */
ltl commit_validity {
    ((all_yes && all_not_suspicious) -> ([] !some_abort)) }
