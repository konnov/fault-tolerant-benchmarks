...
/* variations on the weakly and strongly one-step */
    ...
    /* fine tuning of the intervals for different cases */
        ...
        /* fast0 needs N as a threshold */
        #pragma option "piaDom.thresholds=0,1,moreNminusTdiv2-1,moreNminusTdiv2,moreNplus3Tdiv2,N-T,N"
    #else
        #ifdef STRONGLY_1S
            /* properties one_step{0,1} require the fine-grained domain */
            /* fast0 needs 1 and N as thresholds */
            #pragma option "piaDom.thresholds=0,1,T+1,moreNminusTdiv2,moreNplus3Tdiv2,N-T,N"
        #else
...
atomic all_at_D0 = all(Proc:pc == D0);
atomic all_at_D1 = all(Proc:pc == D1);
...
active[N - F] proctype Proc() {
    ...
    do
        :: atomic {
            ...
            if
                /* first, broadcast your own value, then receive votes */
                :: pc == V0 || pc == V1 ->
                    next_nrcvd0 = 0; next_nrcvd1 = 0;
            	...
            	if
                    ...
                    :: (pc != V0 && pc != V1 && pc != S0 && pc != S1)
                            || (pc != V0 && pc != V1 && next_nrcvd0 + next_nrcvd1 < N - T) ->
                    	next_pc = pc;
            	fi;
            	/* send votes */
            	if
...
ltl fairness { []<>(!in_transit0 && !in_transit1) }
    ...
    ltl fast0 { all_at_V0 -> <>all_at_D0 }
    ltl fast1 { all_at_V1 -> <>all_at_D1 }
