...
symbolic int MAJ; /* auxillary parameter that expresses majority */
...
#ifndef BUG1
assume(((MAJ - 1) * 2 == N - 1) || ((MAJ - 1) * 2 == N));
#else
    /* use floor instead of ceiling, this happens... */
assume(((MAJ - 1) * 2 == N - 2) || ((MAJ - 1) * 2 == N - 1));
#endif

#ifdef CASE4
/* case 4 */
  /* tuning the thresholds for termination */
  /*#pragma option "piaDom.thresholds=0,F,MAJ"*/
#ifndef BUG1
assume(MAJ == N - T);
#else
assume(MAJ == N - T - 1); /* incorrect majority */
#endif
assume (F >= 1);
#else
    #ifdef CASE3
/* case 3 */
  /* tuning the thresholds for termination */
  /*#pragma option "piaDom.thresholds=0,F,MAJ,N-T"*/
assume(MAJ < N - T);
assume (F >= 1);
    #else
        #ifdef CASE2
...
assume (F == 0);
        #else
        /* case 1 */
        /*#pragma option "piaDom.thresholds=0,T,MAJ,N-T" */
assume(MAJ < N - T);
assume (F == 0);
        #endif
    #endif
...
atomic in_transit00plus01 = some(Proc:nrcvd00plus01 < nsnt00plus01);

atomic fairV0andV1 = all(Proc:pc != V0 && Proc:pc != V1);
...
active[N] proctype Proc() {
    ...
    int nrcvd00plus01 = 0, next_nrcvd00plus01 = 0;
    ...
    do
        :: atomic {
                ...
                :: (pc == P0) ->
                    ...
                    havoc(next_nrcvd00plus01);
                    assume (nrcvd00plus01 <= next_nrcvd00plus01
                        && next_nrcvd00plus01 <= nsnt00plus01);
#else
                    if
                      :: (nrcvd00 < nsnt00) ->
                    ...
                    if
                      :: (nrcvd00plus01 < nsnt00plus01) ->
                          next_nrcvd00plus01 = nrcvd00plus01 + 1;
                      :: next_nrcvd00plus01 = nrcvd00plus01;
                    fi;
#endif                    
   	            :: pc == DMZ ->
                    /* optimization phase 0 messages are no use, discard them */
                    next_nrcvd00 = 0; next_nrcvd01 = 0;
                    next_nrcvd00plus01 = 0;
                    
                :: else -> next_nrcvd00 = nrcvd00;
                    next_nrcvd01 = nrcvd01;
                    next_nrcvd00plus01 = nrcvd00plus01;
                fi;
                if
            	...
            	if
                    ...
                    :: pc == DMZ ->
                        next_pc = P1;
                    :: (pc == P1) && (next_nrcvd10 >= MAJ) ->
                    	next_pc = AC0;
                    :: (pc == P1) && (next_nrcvd11 >= MAJ) ->
                    	next_pc = AC1;
                    :: nfaulty < F && pc != CR ->
                        nfaulty++;
                        next_pc = CR;

                    /* introduce self-loops in all states except those
                       where the processes are forced to make progress */
                    :: pc == AC0 || pc == AC1 ->
                    	next_pc = pc;
                    ...
                    :: pc == P1 && next_nrcvd10 < MAJ && next_nrcvd11 < MAJ ->
                    	next_pc = pc;
            	fi;
            ...
            nrcvd00plus01 = next_nrcvd00plus01;
...
#ifdef BUG2
  /* introduce a fairness problem */
  ltl fairness { []<>(!in_transit00 && !in_transit01 && !in_transit10) }
#else
  ltl fairness { []<>(!in_transit00 && !in_transit01
        && !in_transit10 && !in_transit11 && !in_transit00plus01) }
#endif

ltl fairness2 { []<>fairV0andV1 }
